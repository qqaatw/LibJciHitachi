...
class TestAWSAPI:
    def test_login(self, fixture_aws_mock_api):
        ...
        with patch("JciHitachi.aws_connection.GetUser.get_data") as mock_get_data_get_user, \
            patch("JciHitachi.aws_connection.GetUser.login") as mock_login_get_user, \
            patch("JciHitachi.aws_connection.ListSubUser.get_data") as mock_get_data_list_subuser, \
            patch("JciHitachi.aws_connection.GetAllDevice.get_data") as mock_get_data_get_all_device, \
            patch("JciHitachi.aws_connection.JciHitachiAWSMqttConnection.connect") as mock_connect_mqtt:
            # initialization
            aws_tokens = api._aws_tokens
            api._aws_tokens = None
            mock_get_data_get_user.return_value = ("OK", AWSIdentity("id", "username", {"attr", "attr"}))
            mock_login_get_user.return_value = ("OK", aws_tokens)
            mock_get_data_list_subuser.return_value = ("OK", 
                {"results": {
                        "FamilyMemberList" : [
    ...
    def test_refresh_status(self, fixture_aws_mock_api):
        ...
        with patch.object(api, "_mqtt") as mock_mqtt:
            ...
            mock_mqtt.execute.return_value = [[thing_name], [thing_name], [thing_name], []]
            ...
            mock_mqtt.execute.return_value = [[thing_name], [thing_name], [BaseException], []]
            ...
            mock_mqtt.execute.return_value = [[BaseException], [thing_name], [thing_name], []]
            ...
            mock_mqtt.execute.return_value = [[thing_name], [BaseException], [thing_name], []]
...
gateway_mac_address: {MOCK_GATEWAY_MAC}"""
    ...
    def test_from_device_names(self):
        ...
        with pytest.raises(AssertionError, match="Some of device_names are not available from the API."):
